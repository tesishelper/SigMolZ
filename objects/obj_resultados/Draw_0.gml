...
draw_rectangle(x_pos, y_120 +470, x_pos+630,y_120 + 472, false);
...
draw_text(x + 350 , y_120 -25 , "ACTIVITY (%) vs TIME (s)");

draw_text(x + 600 , y_120 -30 , "Click on the graph to see the \nresults of activity vs. concentration!");
...
if (mouse_x-x >= 50 && mouse_x-x <=680 && mouse_y -y >= 32 && mouse_y - y <=510){
	
		draw_rectangle(mouse_x, y + 32,mouse_x+1,y + 505, false);
		draw_rectangle(x_pos, mouse_y,x_pos+630,mouse_y+1, false);

		// calcular % de y 

		var porcentaje = ((mouse_y-583)*(-1)/416)*100
		
		//calcular los segundos que indicax
		
		var tiempo = round((mouse_x-66)/10);
			indice = tiempo;
